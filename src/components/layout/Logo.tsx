...
export function Logo({ size = "md", showText = true, linkTo = "/" }: LogoProps) {
  const content = (
    <div className="flex items-center gap-2">
      <div className={`${sizeClasses[size]} rounded-xl bg-black flex items-center justify-center shadow-md hidden sm:flex`}>
        <img src="/favicon.svg" alt="EduSupport" className="h-[60%] w-[60%]" />
      </div>
      {showText && (
